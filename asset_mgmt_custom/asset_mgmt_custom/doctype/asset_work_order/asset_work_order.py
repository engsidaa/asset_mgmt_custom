...
class AssetWorkOrder(Document):
    ...
    def _apply_sla_policy(self):
        ...
        شكوى تقنية معلومات عامة (بلا أصل): تُفضَّل السياسة المُعلَّمة
        is_it_general_policy=1 لو وُجدت (يضبطها مدير الأصول يدوياً من نفس
        شاشة سياسات SLA العادية — بلا حاجة لأي إعداد افتراضي من الكود)
        بدل السياسة المطابقة للأولوية المختارة يدوياً، فمشاكل IT عادة
        أعجل من صيانة فيزيائية بنفس مستوى الأولوية الاسمي. هذا حقل مستقل
        تماماً عن حقل الأولوية (الذي يجب أن يبقى مطابقاً حرفياً لخيارات
        Asset Work Order — لا يمكن استخدام قيمة اسمية مثل "IT" فيه، كان
        هذا المسار قبل هذا الحقل يبحث عن سياسة اسمها الحرفي "IT" رغم أن
        حقل الأولوية لا يقبل هذه القيمة إطلاقاً، فلم يكن ممكناً إنشاء
        هذه السياسة من الشاشة العادية أصلاً — بحث دائماً بلا نتيجة).
        "تعطيل كامل للعمل" يفرض حداً أقصى صارماً فوق ذلك بصرف النظر عن
        أي سياسة.
        """
        ...
        if is_it_general:
            policy = frappe.db.get_value(
                "Asset Maintenance SLA Policy", {"is_it_general_policy": 1},
