...
- **Phase 6 — منصة EAM/CMMS مؤسسية (قيد التنفيذ)**: تعميق التكامل المالي
  (رسملة CWIP، صرف مخزني آلي لقطع الغيار، CapEx مقابل OpEx، شطب/تكهين،
  IFRS 16 Lease، TCO حقيقي)، ومحرك موثوقية متقدم (ISO 14224 PCR، صيانة
  مزدوجة المحفز، AHI/RUL، قطع غيار دوارة، غرامات SLA)، وسلامة/امتثال
  (LOTO، قوائم فحص ديناميكية، ESG/كربون)، وطبقة Headless API
  (`api/v1/mobile.py`, `api/v1/telemetry.py`) لتطبيق Flutter وIoT
  مستقبلاً. **تفصيل التقدم لكل بند يُحدَّث هنا أولاً بأول أثناء التنفيذ.**
