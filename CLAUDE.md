...
- **Phase 8 — تدقيق شامل + جولة ثالثة من المزايا الاحترافية (10 بنود)**:
  بعد تدقيق كودي مُفصَّل لكل مراحل 6/7 (رصد 8 ثغرات فعلية: تقييم صفري
  للقطع الدوارة، تسلسل إلغاء مخزني فاشل، تعدد دفاتر إهلاك، Meter
  Rollover، تكرار زائف في IoT، أولوية صيانة متداخلة، فصل صلاحيات LOTO،
  تقييد أمر عمل الموبايل — كلها أُصلحت)، ثم مراجعة شاملة لكامل الكود
  (67 DocType، 37 تقرير) لاقتراح 10 مزايا جديدة غير مكرَّرة: تفعيل
  مصفوفة الحرجية فعلياً (أولوية/فحص سلامة)، تقييم مورّد تلقائي من بيانات
  حقيقية، PM Compliance% في مؤشر صحة الفرع، كشف انحراف إحصائي (SPC) في
  الحرارة، معالج فتح فرع جديد، تنبيه تجاوز ميزانية CapEx، نقطة إعادة
  طلب ديناميكية لقطع الغيار، ملخص يومي موحّد اختياري، لوحة أصل 360°،
  وطباعة دفعية لملصقات الباركود (عبر آلية Frappe الأصلية
  download_multi_pdf_async، بلا توليد PDF مخصص). كل بند أُعيد استخدام
  بيانات/دوال موجودة فعلياً بدل تكرارها (مثال: get_asset_detail
  المُوسَّعة، نفس منطق تقرير capex_budget_vs_actual، Notification Log
  كمصدر بيانات وحيد للملخص اليومي).
